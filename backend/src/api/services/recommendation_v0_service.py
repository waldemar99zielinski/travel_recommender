...
class RecommendationV0Service:
    def __init__(
        ...
        *,
        ...
        logger.info("Recommendation v0 service initialized")
    ...
    async def chat_stream(
        ...
        logger.info(
            "Recommendation v0 streaming: user_id=%s, session_id=%s",
            session.user_id,
            session.session_id,
        )
        ...
        try:
            async for event_payload in self._recommendation_graph.astream(
                ...
                if not isinstance(event_payload, dict):
                    logger.warning(
                        "Ignoring malformed recommendation_v0 stream payload: %r",
                        event_payload,
                    )
                    continue
                ...
                if not isinstance(event_name, str):
                    logger.warning(
                        "Ignoring recommendation_v0 stream payload without event name: %r",
                        event_payload,
                    )
                    continue
            ...
            logger.info(
                "Recommendation v0 streaming completed: user_id=%s, session_id=%s",
                session.user_id,
                session.session_id,
            )
            ...
            logger.exception(
                "Recommendation v0 streaming failed: user_id=%s, session_id=%s",
