...
class RecommendationV2NoResultsForRecommendationResponseGenerationInput(BaseModel):
    """Input payload for recommendation_v2 responses when no final results remain."""
    ...
    recommendations: list[RecommendationV2] | None = Field(
        None,
        description="Generated recommendation candidates before the final filtering step",
    )
    final_recommendations: list[RecommendationV2] | None = Field(
        None,
        description="Final recommendations after applying the filter, expected to be empty",
    )
