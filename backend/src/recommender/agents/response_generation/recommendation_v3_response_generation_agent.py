...
class RecommendationV3ResponseGenerationInput(BaseModel):
    ...
    outcome: QuerySynthesisRoutingOutcome = Field(..., description="Routing outcome that determines the response strategy")
    reason: str = Field(..., description="Short explanation of why the outcome was chosen")
    synthesized_query: str | None = Field(None, description="Search query used for the recommendation run")
...
class RecommendationV3ResponseGenerationAgentBuilder(BaseAgentBuilder):
    ...
    def build(self) -> RecommendationV3ResponseGenerationAgent:
        ...
        return RecommendationV3ResponseGenerationAgent(llm=llm, prompt=prompt, output_type=output_type)
...
class RecommendationV3ResponseGenerationAgent(BaseAgent):
    ...
    def invoke(self, inputs: RecommendationV3ResponseGenerationInput) -> RecommendationV3ResponseGenerationResult:
        prompt_inputs = self.prompt.format_messages(
            ...
            top_k_recommendations_summary=self._format_recommendations(inputs.top_k_recommendations),
        )
        return super().invoke(prompt_inputs)

    @staticmethod
    def _format_recommendations(recommendations: list[TravelDestinationRecord]) -> str:
        if not recommendations:
            return "None"
        return "\n".join(
            f"name={item.region} (id={item.id}), "
