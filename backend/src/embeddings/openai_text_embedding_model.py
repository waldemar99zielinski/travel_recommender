...
class OpenAITextEmbeddingModel(TextEmbeddingModelProtocol):
    """OpenAI-backed embedding provider used across storage and recommender."""
    ...
    def __init__(
        ...
        *,
        ...
        if configuration.provider != "openai":
            raise ValueError("provider must be 'openai' for OpenAITextEmbeddingModel")

        model_name = configuration.model_name.strip()
        if not model_name:
            raise ValueError("model_name must not be empty")
        api_key = configuration.api_key.strip() if configuration.api_key is not None else None
        if not api_key:
            raise ValueError("api_key must not be empty for OpenAI embeddings")
        ...
        logger.info(
            "Initializing OpenAITextEmbeddingModel: model=%s base_url=%s has_api_key=%s custom_backend=%s",
            self.model_name,
            base_url,
            bool(api_key),
        ...
        if backend is not None:
            ...
            if base_url is not None:
                backend_parameters["base_url"] = base_url

            self.backend = OpenAIEmbeddings(**backend_parameters)
            logger.verbose("Created OpenAIEmbeddings backend from runtime configuration")

        if not self.check_health():
            raise RuntimeError(
                "OpenAITextEmbeddingModel health check failed during initialization. "
                "Verify EMBEDDINGS_MODEL_NAME, EMBEDDINGS_API_KEY, and OpenAI availability."
            )

        logger.info(
            "OpenAITextEmbeddingModel health check passed: model=%s dimensions=%s",
            self.model_name,
            self.get_dimentions(),
        )
    ...
    def check_health(self) -> bool:
        """Return whether OpenAI embedding calls succeed with consistent dimensions."""
